...
void run_worker(char *dirname, int in, int out) {
  ...
  if ((filenames_fp = fopen(strcat(filenames_file_path, "/filenames"), "r")) == NULL) {
  ...
  while(Read(in, query_word, MAXWORD) > 0) {
    ...
    for (int i = 0; i < num_of_files; i++) {
      if(frequencies[i].freq > 0) {
        // printf("%d\t%s\n", frequencies[i].freq, frequencies[i].filename);
        Write(out, &(frequencies[i]), sizeof(FreqRecord)); // TODO: weird display of freq when writing stdout
      }
    }
  }
  Close(in);
